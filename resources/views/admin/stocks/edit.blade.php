                            ...
                            <div class="col-md-8">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" data-toggle="input-mask"
                                        data-mask-format="00/00/0000" name="price" value="{{ $data->price }}">
                                    <span class="font-13 text-muted">e.g "DD/MM/YYYY"</span>
                                </div>
                                <div class="form-group">
                                    <label>Quantity</label>
                                    <input type="number" class="form-control" data-toggle="input-mask"
                                        data-mask-format="00/00/0000" name="quantity" value="{{ $data->quantity }}">
                                    <span class="font-13 text-muted">e.g "DD/MM/YYYY"</span>
                                </div>
                                        ...
                                        @if (!empty($allCapacity))
                                            ...
                                            @foreach ($allCapacity as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ old('capacity_id') == $item->id || $data->capacity_id == $item->id ? 'selected' : false }}>
                                    ...
                                    <select name="product_id" id="" class="form-control">
                                        ...
                                        @if (!empty($allProduct))
                                            ...
                                            @foreach ($allProduct as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ old('product_id') == $item->id || $data->product_id == $item->id ? 'selected' : false }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        @endif
                                        }
